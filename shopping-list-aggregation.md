...
核心逻辑位于 `mealie/services/household_services/shopping_lists.py#L323-L411` 的 `get_shopping_list_items_from_recipe` 方法。
...
`add_recipe_ingredients_to_list` 方法位于 `mealie/services/household_services/shopping_lists.py#L413-L455`，支持批量添加多个食谱：
...
`can_merge` 方法位于 `mealie/services/household_services/shopping_lists.py#L45-L71`。
...
`merge_items` 方法位于 `mealie/services/household_services/shopping_lists.py#L73-L128`。
...
数量换算使用 `mealie/services/parser_services/parser_utils/unit_utils.py` 中的 `UnitConverter` 类和 `merge_quantity_and_unit` 函数。
...
前端状态位于 `frontend/app/composables/shopping-list-page/sub-composables/use-shopping-list-state.ts`。
...
位于 `frontend/app/composables/shopping-list-page/sub-composables/use-shopping-list-crud.ts`。
...
位于 `frontend/app/composables/use-shopping-list-item-actions.ts`。
...
位于 `mealie/schema/household/group_shopping_list.py#L58-L76`。
